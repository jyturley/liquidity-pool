...
import { SpaceCoin, SpaceCoinICO, SpaceCoinICO__factory, SpaceCoin__factory } from "../typechain";
...
// funds 150k ETH
const fundICOSeedGoal = async (ico: SpaceCoinICO, deployer: SignerWithAddress) => {
...
// funds 15k ETH
const fundICOGeneralGoal = async (ico: SpaceCoinICO, deployer: SignerWithAddress) => {
  var wallets: SignerWithAddress[] = await ethers.getSigners();
  for (var x = 0; x < 15; x++) {
...
describe("SpaceCoin", () => {
  ...
  let uniswap: SignerWithAddress;

  let SpaceCoin: SpaceCoin__factory;
  let spc: SpaceCoin;

  beforeEach(async () => {
    [deployer, treasury, icoTreasury, alice, bob, whitelistChris] = await ethers.getSigners();
  ...
  describe("Token Contract", () => {
    ...
    it("Allocates 150,000 SPC of supply to ICO investors (30,000 ETH worth)", async () => {
      expect(await spc.balanceOf(icoTreasury.address)).to.equal(ethers.utils.parseEther("150000"));
    });
    it("Stores the remaining 350,000 SPC of supply in the treasury", async () => {
      expect(await spc.balanceOf(treasury.address)).to.equal(ethers.utils.parseEther("350000"));
    });
    it("Transfers maintain total supply and balances changed for from and to", async () => {
      ...
      expect(await spc.connect(treasury).transfer(alice.address, TEN_TOKENS)).to.be.ok;
  ...
  describe("ICO Contract", () => {
    ...
    beforeEach(async () => {
      ...
      ico = (await SpaceCoinICO.deploy(deployer.address, treasury.address)) as SpaceCoinICO;
      await ico.deployed();
    ...
    describe("Deployment", () => {
      ...
      it("Unable to add zero address as treasury", async () => {
        ...
        await expect(spcICO.deploy(deployer.address, ZERO_ADDRESS)).to.be.revertedWith(
          "Cannot use zero address"
        );
      });
      it("Unable to add zero address as manager", async () => {
        const spcICO = await ethers.getContractFactory("SpaceCoinICO");
        await expect(spcICO.deploy(ZERO_ADDRESS, deployer.address)).to.be.revertedWith(
          "Cannot use zero address"
        );
      });
      it("Emits SpaceCoinDeployedAndMinted when deployed", async () => {
        expect(await SpaceCoinICO.deploy(deployer.address, treasury.address)).to.emit(
          ico,
          "SpaceCoinDeployedAndMinted"
        );
      });
    });
    describe("Management", () => {
      ...
      it("Prevents non-owners from advancing phase forward", async () => {
        ...
        expect(await ico.currentPhase()).to.equal(SEED_PHASE);
        await expect(ico.connect(alice).beginOpenPhase()).to.be.revertedWith("Invalid permissions");
        expect(await ico.currentPhase()).to.equal(SEED_PHASE);
      });
      ...
      it("Emits ICONowPaused event when paused", async () => {
        await expect(ico.connect(deployer).pauseICO()).to.emit(ico, "ICONowPaused");
      });
      it("Emits ICONowActive event when unpaused", async () => {
        await ico.connect(deployer).pauseICO();
        await expect(ico.connect(deployer).resumeICO()).to.emit(ico, "ICONowActive");
      });
      it("Prevents non-owners from pausing or resuming funding at any time", async () => {
        await expect(ico.connect(alice).pauseICO()).to.be.revertedWith("Invalid permissions");
        expect(await ico.isPaused()).to.be.false;
      });
      it("Allows owner to add seed investors to the whitelist", async () => {
        expect(await ico.isWhitelisted(alice.address)).to.be.false;
        expect(await ico.connect(deployer).addToWhitelist(alice.address)).to.be.ok;
      ...
      it("Prevents non-owners from adding a single seed investor to the whitelist", async () => {
        await expect(ico.connect(alice).addToWhitelist(alice.address)).to.be.revertedWith(
          "Invalid permissions"
        );
        expect(await ico.isWhitelisted(alice.address)).to.be.false;
      });
      it("Prevents adding an invalid address to whitelist", async () => {
        await expect(ico.connect(deployer).addToWhitelist(ZERO_ADDRESS)).to.be.revertedWith(
          "Invalid account for whitelist"
        );
        await expect(ico.isWhitelisted(ZERO_ADDRESS)).to.be.revertedWith("Invalid account");
      });
      it("Prevents adding to whitelist in General phase", async () => {
        await ico.connect(deployer).beginGeneralPhase();
        await expect(ico.connect(deployer).addToWhitelist(bob.address)).to.be.revertedWith(
    ...
    describe("Space Pool", () => {
      let uniswapTokenAmtBefore: BigNumber;
      let treasuryTokenAmtBefore: BigNumber;
      beforeEach(async () => {
        const signers = await ethers.getSigners();
        uniswap = (await ethers.getSigners())[34];
        uniswapTokenAmtBefore = await spc.balanceOf(uniswap.address);
        await spc.connect(treasury).approve(ico.address, TOTAL_SUPPLY);
        if (!uniswapTokenAmtBefore.eq(0)) {
          await spc.connect(uniswap).approve(treasury.address, ETH("100000"));
          await spc.transferFrom(uniswap.address, treasury.address, uniswapTokenAmtBefore);
        }
        treasuryTokenAmtBefore = await spc.balanceOf(treasury.address);
      });
      it("Manager can withdraw tokens to a given address", async () => {
        expect(await ico.connect(deployer).withdrawTokens(uniswap.address, ETH("10"))).to.be.ok;
        expect(await spc.balanceOf(uniswap.address)).to.equal(uniswapTokenAmtBefore.add(ETH("10")));
        expect(await spc.balanceOf(treasury.address)).to.equal(
          treasuryTokenAmtBefore.sub(ETH("10"))
        );
      });
      it("Withdrawing emits 'TokensWithdrawn' event", async () => {
        await expect(ico.connect(deployer).withdrawTokens(uniswap.address, ETH("10")))
          .to.emit(ico, "TokensWithdrawn")
          .withArgs(uniswap.address, ETH("10"));
      });
      it("Prevents non-members from withdrawing", async () => {
        await expect(
          ico.connect(alice).withdrawTokens(uniswap.address, ETH("10"))
        ).to.be.revertedWith("Invalid permissions");
      });
      it("Prevents withdrawing to a zero address", async () => {
        await expect(
          ico.connect(deployer).withdrawTokens(ZERO_ADDRESS, ETH("10"))
        ).to.be.revertedWith("ERC20: transfer to the zero address");
      });
      it("Prevents withdrawing zero tokens", async () => {
        await expect(
          ico.connect(deployer).withdrawTokens(uniswap.address, ETH("0"))
        ).to.be.revertedWith("Invalid token amount");
      });
      it("30k ETH and 150k SPC able to be withdrawn to the pool simultaneously.", async () => {
        // setup
        const totalETHRaised = ETH("30000");
        const totalSPCRaised = totalETHRaised.mul(5);
        const wallets = await ethers.getSigners();
        const david = wallets[31];
        const eric = wallets[32];
        const frank = wallets[33];

        const numTokensStartForICO = await spc.balanceOf(ico.address);
        expect(numTokensStartForICO).to.be.equal(ethers.utils.parseEther("150000"));
        ...
        expect(await ico.weiRaised()).to.equal(ethers.utils.parseEther("30000"));
        expect(await spc.balanceOf(uniswap.address)).to.equal(0);
        expect(treasuryTokenAmtBefore).to.equal(ETH("350000"));
        expect(await ethers.provider.getBalance(treasury.address)).to.gte(totalETHRaised);
      });
    });
    describe("Contributions & Redemptions", () => {
      ...
      describe("Seed Phase", () => {
        it("Allows contributions through sending eth to contract", async () => {
          ...
          expect(await ico.contributions(whitelistChris.address)).to.equal(ONE_ETHER);
          treasuryEndBalance = await treasury.getBalance();
          expect(await ico.weiRaised()).to.equal(treasuryEndBalance.sub(treasuryStartBalance));
        });
        it("Allows contributions from whitelisted investors", async () => {
          expect(await ico.buyTokens(whitelistChris.address, { value: ONE_ETHER })).to.be.ok;
          expect(await ico.contributions(whitelistChris.address)).to.equal(ONE_ETHER);
          expect(await ico.tokensToCollect(whitelistChris.address)).to.equal(FIVE_TOKENS);
        });
        it("Blocks non-whitelist investors via direct transfer", async () => {
          ...
          ).to.be.revertedWith("Only whitelisted addresses can buy tokens at this phase");
        });
        it("Blocks contributions above individual Limit from whitelisted investors", async () => {
          ...
          ).to.be.revertedWith("Contribution exceeds Seed Phase individual limit");
        ...
        it("Blocks contributions above funding round limit from whitelisted investors", async () => {
          ...
          expect(await ico.weiRaised()).to.equal(ethers.utils.parseEther("15000"));
          await expect(
            ico.buyTokens(whitelistChris.address, {
              value: ONE_ETHER,
            })
          ).to.be.revertedWith("Contribution exceeds Seed Phase total contribution limit");
          expect(await ico.weiRaised()).to.equal(ethers.utils.parseEther("15000"));
        });
        it("Blocks contributions from non-whitelisted investors", async () => {
          await expect(ico.buyTokens(alice.address, { value: ONE_ETHER })).to.be.revertedWith(
        ...
        it("Emits a TokensPurchased event after a contribution is made from a whitelisted investor", async () => {
          await expect(ico.buyTokens(whitelistChris.address, { value: ONE_ETHER }))
            .to.emit(ico, "TokensPurchased")
            .withArgs(deployer.address, whitelistChris.address, ONE_ETHER, FIVE_TOKENS);
          await expect(
            ...
            .to.emit(ico, "TokensPurchased")
            .withArgs(whitelistChris.address, whitelistChris.address, ONE_ETHER, FIVE_TOKENS);
        });
        ...
        it("Prevents token redemptions", async () => {
          ...
          await expect(ico.connect(whitelistChris).redeemTokens()).to.be.revertedWith(
            "Must be Open Phase to redeem tokens"
          );
          expect(await ico.tokensToCollect(whitelistChris.address)).to.be.equal(FIVE_TOKENS);
        });
      ...
      describe("General Phase", () => {
        ...
        it("Allows contributions from whitelisted investors", async () => {
          await ico.connect(deployer).beginGeneralPhase();
          expect(await ico.currentPhase()).to.equal(GENERAL_PHASE);
          expect(await ico.buyTokens(alice.address, { value: ONE_ETHER })).to.be.ok;
          expect(await ico.tokensToCollect(alice.address)).to.equal(FIVE_TOKENS);
          expect(await ico.weiRaised()).to.equal(ONE_ETHER);
        });
        it("Allows contributions from non-whitelisted investors", async () => {
          await ico.connect(deployer).beginGeneralPhase();
          expect(await ico.connect(bob).buyTokens(bob.address, { value: ONE_ETHER })).to.be.ok;
        ...
        it("Blocks contributions from seed investors who are already above general individual limit", async () => {
          ...
          await ico.connect(deployer).beginGeneralPhase();
          expect(await ico.weiRaised()).to.equal(ethers.utils.parseEther("1500"));
          await expect(
            ico.connect(whitelistChris).buyTokens(whitelistChris.address, {
              value: ONE_ETHER,
            })
          ).to.be.revertedWith("Contribution exceeds Seed General individual limits");
          expect(await ico.weiRaised()).to.equal(ethers.utils.parseEther("1500"));
          expect(await ico.contributions(whitelistChris.address)).to.equal(
            ethers.utils.parseEther("1500")
          );
        ...
        it("Blocks contributions above individual limit", async () => {
          ...
          expect(await ico.contributions(alice.address)).to.equal(ethers.utils.parseEther("980"));
          expect(await ico.weiRaised()).to.equal(ethers.utils.parseEther("980"));
        });
        ...
        it("Blocks contributions when fundraising is paused", async () => {
          ...
          await expect(ico.connect(alice).buyTokens(alice.address, { value: ONE_ETHER })).to.not
            .reverted;
        });
        it("Prevents token redemptions", async () => {
          await ico.connect(deployer).beginGeneralPhase();
      ...
      describe("Open Phase", () => {
        it("Automatically redeems new contributions for tokens", async () => {
          await ico.connect(deployer).beginGeneralPhase();
          await ico.connect(deployer).beginOpenPhase();
          expect(await ico.connect(alice).buyTokens(alice.address, { value: ONE_ETHER })).to.be.ok;
          expect(await ico.tokensToCollect(alice.address)).to.equal(0);
          expect(await ico.weiRaised()).to.equal(ONE_ETHER);
          expect(await spc.balanceOf(alice.address)).to.equal(FIVE_TOKENS);

          expect(await bob.sendTransaction({ to: ico.address, value: ONE_ETHER })).to.be.ok;
        ...
        it("Emits a TokensPurchased event after a contribution is made", async () => {
          await ico.connect(deployer).beginGeneralPhase();
          await ico.connect(deployer).beginOpenPhase();
          await expect(ico.connect(alice).buyTokens(alice.address, { value: ONE_ETHER }))
            .to.emit(ico, "TokensPurchased")
            .withArgs(alice.address, alice.address, ONE_ETHER, FIVE_TOKENS);
          await expect(
            alice.sendTransaction({
          ...
          expect(await ico.contributions(alice.address)).to.equal(ethers.utils.parseEther("2"));
          expect(await ico.tokensToCollect(alice.address)).to.equal(0);
        });
        it("Blocks contributions when fundraising is paused", async () => {
          await ico.connect(deployer).beginGeneralPhase();
        ...
        it("Emits a TokensRedeemed event after tokens are redeemed", async () => {
          await ico.connect(deployer).beginGeneralPhase();
          await ico.connect(alice).buyTokens(alice.address, { value: ONE_ETHER });
          await ico.connect(deployer).beginOpenPhase();
        ...
        it("Prevents unearned token redemptions", async () => {
          await ico.connect(deployer).beginGeneralPhase();
          await ico.connect(bob).buyTokens(bob.address, { value: ethers.utils.parseEther("50") });
          await ico.connect(deployer).beginOpenPhase();
          await expect(ico.connect(alice).redeemTokens()).to.be.revertedWith(
        ...
        it("Allows individual contributions greater than 1500", async () => {
          ...
          expect(await spc.balanceOf(alice.address)).to.equal(ethers.utils.parseEther("25000"));
          expect(await ico.contributions(alice.address)).to.equal(ethers.utils.parseEther("5000"));
          expect(await ico.tokensToCollect(alice.address)).to.equal(0);
          expect(await ico.weiRaised()).to.equal(ethers.utils.parseEther("5000"));
        });
        it("Moves sales to treasury address", async () => {
          await ico.connect(deployer).beginGeneralPhase();
          await ico.connect(deployer).beginOpenPhase();
        ...
        it("Given away all tokens when ICO is fully funded", async () => {
          ...
          const wallets = await ethers.getSigners();
          await uniswap.sendTransaction({
            to: wallets[24].address,
            value: ETH("10000"),
          });
          await uniswap.sendTransaction({
            to: wallets[25].address,
            value: ETH("10000"),
          });
          await uniswap.sendTransaction({
            to: wallets[26].address,
            value: ETH("10000"),
          });
          const david = wallets[24];
          const eric = wallets[25];
          const frank = wallets[26];
          ...
          const rate = 5;
          expect(await spc.balanceOf(david.address)).to.equal(almostTenGrand.mul(rate));
          expect(await spc.balanceOf(eric.address)).to.equal(almostTenGrand.mul(rate));
          expect(await spc.balanceOf(frank.address)).to.equal(almostTenGrand.mul(rate));
