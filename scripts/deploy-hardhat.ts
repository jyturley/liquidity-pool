...
async function main() {
  ...
  const fundICOSeedGoal = async (
    ...
    for (var x = 0; x < 10; x++) {
      let i = x + 8; // first eight for saved for testing
  ...
  const fundICOGeneralGoal = async (
    ...
    var wallets: SignerWithAddress[] = await ethers.getSigners();
    for (var x = 0; x < 15; x++) {
      let i = x + 10 + 8; // ten from fundICOSeedGoal() and six saved for testing
      await ico.connect(wallets[i]).buyTokens(wallets[i].address, {
  ...
  console.log("-------------");
  [deployer, alice, bob, chris, david, spcManager, spcTreasury, ...signers] =
  ...
  await ico.connect(deployer).addToWhitelist(bob.address);

  await fundICOSeedGoal(ico, deployer);
  await ico.connect(deployer).beginGeneralPhase();
  await fundICOGeneralGoal(ico, deployer);
  const ethRaised = ethers.utils.formatEther(await ico.weiRaised());
  console.log(`Raised ${ethRaised} ETH total`);
  console.log(
    `SPC token balance for treasury ${await spc.balanceOf(spcTreasury.address)}`
  );
  console.log(
    `SPC token balance for alice ${await spc.balanceOf(alice.address)}`
  );
}
